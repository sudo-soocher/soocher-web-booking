...
// ============================================================
// SCHEDULED CRON: Consultation Reminders
// Runs every minute to send FCM and WhatsApp reminders
// at 10, 5, and 1 minute before the consultation starts.
// ============================================================

// Helper to normalize time to epoch ms
...
exports.consultationReminderCron = onSchedule(
  ...
  async (event) => {
    ...
    const col = admin.firestore().collection("Consultations");
    const numSnap = await col
      .where("consultationTime", ">=", now)
      .where("consultationTime", "<=", windowEnd)
      .get();

    const tsSnap = await col
      .where("consultationTime", ">=", admin.firestore.Timestamp.fromMillis(now))
      .where("consultationTime", "<=", admin.firestore.Timestamp.fromMillis(windowEnd))
      .get();

    const byId = new Map();
    numSnap.docs.forEach((d) => byId.set(d.id, d));
    tsSnap.docs.forEach((d) => byId.set(d.id, d));

    if (byId.size === 0) return;

    for (const doc of byId.values()) {
      const data = doc.data();
      const consultationTimeMs = toEpochMs(data.consultationTime);
      if (!consultationTimeMs) continue;

      const minutesRemaining = Math.round((consultationTimeMs - now) / 60000);
      const consultationId = doc.id;
      const participants = data.participants || [];
      const patientUid = participants[0];
      const doctorUid = participants[1];

      // ---- FCM branch: 10 / 5 / 1 minutes before ----
      let title, body, flagField;
      if (minutesRemaining === 10 && !data.noti_10min) {
        title = "⏰ Appointment Reminder";
        body = "Your appointment starts in 10 minutes!";
        flagField = "noti_10min";
      } else if (minutesRemaining === 5 && !data.noti_5min) {
        title = "⏰ Appointment Reminder";
        body = "Your appointment starts in 5 minutes. Get ready!";
        flagField = "noti_5min";
      } else if (minutesRemaining === 1 && !data.noti_1min) {
        title = "🚨 Appointment Starting Soon";
        body = "Your appointment starts in 1 minute!";
        flagField = "noti_1min";
      }

      if (title && flagField) {
        const [patientSnap, doctorSnap] = await Promise.all([
          patientUid ? admin.firestore().collection("Users").doc(patientUid).get() : null,
          doctorUid ? admin.firestore().collection("Users").doc(doctorUid).get() : null,
        ]);
        ...
        try {
          ...
          console.log(`Sent ${minutesRemaining}min reminder for ${consultationId}`);
        } catch (e) {
          console.error(`FCM ${minutesRemaining}min failed:`, e);
        }
      }
    }
  }

# Cloud Functions — WhatsApp Notifications Removed

This is the cleaned `functions/index.js` for the `soocherv2` Firebase project. All WhatsApp notification sending has been removed; FCM push, Cloud Tasks scheduling, Zoho sync, Prisma API calls, Razorpay, and Stream features are preserved.
...
// Notification for booked appointment (WhatsApp sends removed; FCM + Cloud Task reminder retained)
...
// Notification Scheduler (WhatsApp reminders removed; FCM push retained)
...
1. **`sendWhatsappNotification` helper** (Meta Graph API caller) — deleted entirely.
2. **Inside `sendBookedAppointmentNotification`** — removed `phoneNumberArray` collection, the per-recipient WhatsApp loop, and the hardcoded `919539656390` (Reshma) confirmation. Cloud Tasks reminder scheduling kept.
3. **Inside `onScheduledNotificationCallback`** — removed all 3 WhatsApp reminder sends (doctor, patient, Reshma), plus the now-unused phone-number lookups and consultation re-fetch. FCM push notifications kept.
4. **OneSignal config block** — unused, removed.
5. **Orphan helpers `getRecordIdByName` and `convertDateToZohoFormat`** — only ever called from the (commented-out) Zoho Deals push inside the removed WhatsApp block. Removed.
6. Dead variables (`date`, `time`, `doctorName`/`patientName`/`doctorPhoneNumber`/`patientPhoneNumber` in callback).

## What was kept

- All FCM push notifications (chat messages, booking-reminder push, cancellation, feed posts, upvotes, replies)
- Cloud Tasks scheduling for the 5-min-before-consultation reminder
- All Zoho CRM sync (Contacts, Patients) on user create
- All axios calls to the external Prisma backend (`baseUrl2`)
- Razorpay order generation
- Stream call-token generation
- Doctor verification automation
- Speciality and state aggregation

## Before deploying — credential rotation

The hardcoded credentials in this file should be moved to Secret Manager or `functions.config()` before deploying:

- Razorpay live keys (`rzp_live_znKpMWksb4GGN5` + secret)
- Stream API secret
- Zoho client secret
- Old Meta WhatsApp Business token (revoke from Meta Developer Console — it's no longer needed)
...
cd functions
npm install
firebase deploy --only functions --project soocherv2
```

## Test plan

- Booking from Flutter app → FCM push to patient + doctor; no WhatsApp from `9656250403`.
- Booking from web app → WhatsApp confirmation from `8136871565` (ChatMitra) only.
- 5 minutes before a consultation → FCM reminder push only, no WhatsApp.

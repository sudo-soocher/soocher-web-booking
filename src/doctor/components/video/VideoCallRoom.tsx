...
function ConnectingScreen({ status, patientName }: { status: string; patientName: string }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-8 px-6 text-center">
      ...
      <div className="relative flex items-center justify-center">
        ...
        <div className="relative z-10 grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-primary-400 to-primary-700 text-2xl font-black text-white shadow-2xl shadow-primary/40 ring-4 ring-primary/30">
          {getInitials(patientName)}
        </div>
      </div>
      <div>
        <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.2em] text-primary-300">Connecting to</p>
        <h2 className="text-2xl font-black tracking-tight text-white">{patientName}</h2>
      </div>
...
/* ── Call ended screen ───────────────────────────────────────────── */
function CallEndedScreen({ onLeave, patientName }: { onLeave: () => void; patientName: string }) {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <div>
        ...
        <h2 className="text-3xl font-black tracking-tight text-white">Great consultation!</h2>
        <p className="mt-2 text-sm text-white/50">Your session with <span className="font-semibold text-white/80">{patientName}</span> has ended.</p>
      </div>
      <motion.button initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        onClick={onLeave}
        className="flex items-center gap-3 rounded-full bg-gradient-to-r from-primary-500 to-primary-600 px-8 py-4 font-bold text-white shadow-2xl shadow-primary/30 transition-all hover:scale-105">
        <FaStethoscope /> Post-Consult Actions
      </motion.button>
    </motion.div>
  );
}
...
function LiveCallUI({ onLeave, patientName, doctorName }: { onLeave: () => void; patientName: string; doctorName: string }) {
  ...
  if (callingState === CallingState.LEFT) {
    return <CallEndedScreen onLeave={onLeave} patientName={patientName} />;
  }

  const remoteParticipant = remoteParticipants[0];
  const patientJoined = !!remoteParticipant;
  // Patient has video on only if they're actively publishing video
  const patientHasVideo = patientJoined && hasVideo(remoteParticipant);
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-black">
      ...
      <div className="relative flex-1 overflow-hidden">
        {patientJoined ? (
          <div className="relative h-full w-full bg-[#0d0f14]">
            {/* Stream video — always mounted so it doesn't re-init on toggle */}
            <div className={`h-full w-full transition-opacity duration-300 [&_.str-video__participant-view]:h-full [&_.str-video__participant-view]:w-full [&_.str-video__participant-view]:rounded-none ${patientHasVideo ? "opacity-100" : "opacity-0 pointer-events-none absolute inset-0"}`}>
              <ParticipantView participant={remoteParticipant} className="h-full w-full" />
            </div>
            {/* Avatar overlay when patient camera is off */}
            {!patientHasVideo && (
              <div className="flex h-full flex-col items-center justify-center gap-4">
                <div className="grid h-24 w-24 place-items-center rounded-full bg-gradient-to-br from-primary-500 to-primary-800 text-3xl font-black text-white shadow-2xl shadow-primary/30 ring-4 ring-primary/20">
                  {getInitials(patientName)}
                </div>
                <div className="text-center">
                  <p className="text-base font-black text-white">{patientName}</p>
                  <p className="mt-1 flex items-center justify-center gap-1.5 text-xs text-white/40">
                    <FaVideoSlash className="text-[10px]" /> Camera off
                  </p>
                </div>
              </div>
            )}
          ...
          /* Waiting screen — inside the video area so controls are still visible */
          <div className="flex h-full flex-col items-center justify-center gap-4 bg-[#0d0f14]">
            <div className="relative flex items-center justify-center">
              ...
              <div className="relative z-10 grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-primary-600 to-primary-800 text-2xl font-black text-white ring-4 ring-primary/20">
                {getInitials(patientName)}
              </div>
            </div>
            <div className="text-center">
              <p className="text-lg font-black text-white">{patientName}</p>
        ...
        {/* Doctor PiP (bottom-right of video area) */}
        {localParticipant && (
          <div ref={pipRef}
            ...
            {camOff ? (
              <div className="flex h-full w-full flex-col items-center justify-center gap-1.5 bg-slate-800">
                <div className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-primary-500 to-primary-700 text-sm font-black text-white">
                  {getInitials(doctorName)}
                </div>
                <FaVideoSlash className="text-xs text-white/30" />
              </div>
            ) : (
      ...
      {/* ── Control bar (always visible, never overlapped) ── */}
      <div className="shrink-0 bg-[#0d0f14] pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-4">
        <div className="flex items-center justify-center gap-5">

          {/* Mic toggle */}
          <button onClick={toggleMic}
            className={`flex h-14 w-14 flex-col items-center justify-center gap-1 rounded-full transition-all active:scale-95 ${micMuted ? "bg-rose-500/20 ring-1 ring-rose-500/40" : "bg-white/10 ring-1 ring-white/10 hover:bg-white/20"}`}>
            {micMuted
          ...
          <button onClick={toggleCam}
            className={`flex h-14 w-14 flex-col items-center justify-center gap-1 rounded-full transition-all active:scale-95 ${camOff ? "bg-rose-500/20 ring-1 ring-rose-500/40" : "bg-white/10 ring-1 ring-white/10 hover:bg-white/20"}`}>
            {camOff
...
export function VideoCallRoom({ consultation, onLeave }: VideoCallRoomProps) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Explicitly publish camera + mic so the remote participant can see/hear the doctor
  ...
  if (!client || !call) return <ConnectingScreen status={status} patientName={patientName} />;

  return (
    <StreamVideo client={client}>
      <StreamCall call={call}>
        <LiveCallUI onLeave={onLeave} patientName={patientName} doctorName={doctorName} />
      </StreamCall>
    </StreamVideo>
  );
}

...
export default function HomeClient({
  ...
  /** Fetched server-side in page.tsx so the grid is in the very first HTML
   *  response instead of waiting on a client-side Firestore round trip. */
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [loading, setLoading] = useState(initialSpecialities.length === 0);
  ...
  */
  ...
  useEffect(() => {
    // The server already provided this list in the initial HTML — only fall
    // back to the client-side fetch (with its own cache + retry) when that
    // came back empty, e.g. the server-side read failed or timed out.
  ...
  }, [loadSpecialities, initialSpecialities]);

...
export default function DoctorsClient({
    ...
    /** Selected speciality resolved server-side from ?speciality=. */
    initialSpeciality: string;
    /** Fetched server-side so the filter dropdown isn't empty on first paint. */
    initialSpecialities: string[];
    /** Fetched server-side for `initialSpeciality`, so the card grid is in the
     *  very first HTML response instead of waiting on a client Firestore
     *  round trip. */
    ...
    const [specialities, setSpecialities] = useState<string[]>(initialSpecialities);
    ...
    useEffect(() => {
        if (initialSpecialities.length > 0) return;
        let cancelled = false;
    ...
    // The server already provided the initial speciality's doctors in the
    // HTML — only run the client fetch on a later speciality change (or if
    // the server-side read came back empty, e.g. it failed/timed out).
